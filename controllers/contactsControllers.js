...
const { addContactJoi, updateContactJoi } = require("../joi/joi");
...
const createContact = tryCatchWrapper(async (req, res) => {
  const { error, value } = addContactJoi.validate(req.body);
  console.log(addContactJoi.validate(req.body));
  if (error) {
...
const updateContact = tryCatchWrapper(async (req, res) => {
  ...
  if (name || email || phone) {
    const { error } = updateContactJoi.validate(req.body);
    if (error) {
      return res.status(400).json({ message: error.details[0].message });
    }
